...
```{r}
sample_df <- sample_df %>%
  mutate(item = fct_infreq(as_factor(item))) %>%
  mutate(item_lab = fct_infreq(as_factor(strtrim(items, width = 14))))
  

ggplot(data = sample_df) +
  geom_bar(aes(x = item_lab)) +
  coord_flip() +
  hrbrthemes::theme_ipsum_rc() +
  labs(title = "TSA Dangerous Items FY2015",
       caption = dataset_key)
```
